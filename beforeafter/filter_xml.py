...
if __name__ == "__main__":
    ...
    parser.add_argument('--files', help='will use glob, needs .info.xml output from caevo, hint:use quotes')
    ...
    main(args.files, out_dir)

def trees_to_out(basename, trees, connective):
    if len(trees) > 0:
        with open(basename + "_" + connective + ".tree", "w+") as outfile:
            for tree in trees:
